...
public class Breakout extends WindowProgram {
    ...
    private void playGame() {
        ...
        for (int i = 0; i < NTURNS && !isGameWon(bricksBrokenCounter); i++) {
            ...
            waitForClick();
            bricksBrokenCounter = playRound(ball, bricksBrokenCounter);
        }
     ...
     */
    private int playRound(GOval ball, int bricksBrokenCounter) {
        setVX();

        while (!ballHitsBottom(ball) && !isGameWon(bricksBrokenCounter)) {
            ball.move(vx, vy);
            pause(PAUSE_TIME);
            bricksBrokenCounter = updateScene(ball, bricksBrokenCounter);
        }
     ...
     * @return The broken bricks' number.
     */
    private int updateScene(GOval ball, int bricksBrokenCounter) {
        ...
        if (ballHitsHorizontalEdge(ball)) { // left/right
            ...
            vy = -vy;
        } else if (collider != paddle && collider != null) { // brick
    ...
    private GLabel createReport(String reportText, Color color) {
        ...
        double reportXOffset = getWidth() / 2.0 - report.getWidth() / 2;
